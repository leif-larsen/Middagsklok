...
internal sealed class UseCase(AppDbContext dbContext)
{
    ...
    // Executes the dish creation workflow.
    public async Task<UseCaseResult> Execute(Request request, CancellationToken cancellationToken)
    {
        ...
        {
            var invalidResult = new UseCaseResult(CreateOutcome.Invalid, null, validation.Errors);
        ...
        var hasDuplicate = await _dbContext.Dishes
            .AsNoTracking()
            .AnyAsync(
                dish => dish.Name.ToUpper() == normalizedName,
                cancellationToken);
        ...
        if (hasDuplicate)
        {
            ...
            var conflictResult = new UseCaseResult(CreateOutcome.Conflict, null, new[] { conflictError });
        ...
        {
            var invalidResult = new UseCaseResult(CreateOutcome.Invalid, null, missingErrors);
            return invalidResult;
        }
        ...
        var response = MapDish(dishEntity, ingredientLookup);
        var result = new UseCaseResult(CreateOutcome.Success, response, Array.Empty<ValidationError>());
